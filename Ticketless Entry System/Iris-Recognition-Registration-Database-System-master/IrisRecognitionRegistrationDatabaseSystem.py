...
class InputDialog_Register(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        layout.addRow("Aadhar number:", self.fifth)
...
class InputDialog_Verify(QDialog):
    ...
    def set_image(self, image_frame, image_frame2, idnum, namen, agen, gendern,adhar):
        self.imageLabel2.setPixmap(QtGui.QPixmap(image_frame2))
        self.imageLabel.setPixmap(QtGui.QPixmap(image_frame))
...
#==================================== GUI FOR PROGRAM ====================================
    ...
    def register_dialog_open_verify(self):
        inp1 = QtWidgets.QInputDialog(self)
        inp1.setInputMode(QtWidgets.QInputDialog.TextInput)
        inp1.setFixedSize(600, 100)

        
        #GETTING INPUT TO VERIFY.
        inp1.setWindowTitle('Confirmation')
        inp1.setLabelText('Enter Id:')

        if inp1.exec_() == QtWidgets.QDialog.Accepted:
            #print(inp.textValue())
            global verify_id
            verify_id = inp1.textValue()
    ...
    def show_user_verification_info(self):
        ...
        for f_name in os.listdir('database'):
            ...
            if verify_id.strip() == get_id_from_database[0].strip():
                ...
                iris_aadhar = get_id_from_database[4].strip()
                break
        ...
        if is_matched == True and verify_id.strip() == get_id_from_database[0].strip() :
            if self.image_frame2 is not None:
                ...
                self.uiverify.set_image('database\\' + iris_f_name, 'database\\' + iris_f_name, iris_id, iris_name, iris_age, iris_gender,iris_adhar) 
                self.uiverify.exec()
        else:
    ...
    def VerifyEyes(self):
        if self.is_register == 0:
            ...
            if self.is_verify == 1:
                ...
                #close popup
                self.popup_window.close()
                self.VerifyUserButton.setStyleSheet('QPushButton {background-color: #E1E1E2}')
    ...
    def ShowAllRegisteredUsers(self): 
        ...
        col_headers = ['IDENTIF. #', 'NAME.','AGE.', 'GENDER.', 'DATE REGIST.', 'CURRENT REGIST.', 'IMAGE.']
        ...
        for f_name in os.listdir('database'):
            ...
            infobutton = QPushButton("IRIS IMAGE")
